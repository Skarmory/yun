...
struct MapCell* map_cell_new(int cell_x, int cell_y)
{
    ...
    map_cell_init(cell, cell_x, cell_y); 
    return cell;
}

void map_cell_free(struct MapCell* cell)
{
    array_uninit(&cell->rooms);
...
void map_cell_init(struct MapCell* cell, int cx, int cy)
{
    cell->cell_x = cx;
    cell->cell_y = cy;
    cell->world_x = cx * g_map_cell_width;
    cell->world_y = cy * g_map_cell_height;
    array_init(&cell->rooms, sizeof(struct Room), 10, NULL, NULL);
    ...
    //    list_init(&loc->obj_list);
        loc->terrain = C_NULL_ENTITY_HANDLE;
        loc->creature = C_NULL_ENTITY_HANDLE;
        loc->feature = C_NULL_ENTITY_HANDLE;
        stackarray_init(&loc->objects);
    }
}
